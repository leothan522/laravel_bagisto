        ...
        <div class="mt-3.5 flex gap-2.5 max-xl:flex-wrap">
            ...
            <div class="flex flex-1 flex-col gap-2 max-xl:flex-auto">
                ...
                @if ($order->payment->method === 'moneytransfer')
                    <div class="box-shadow rounded bg-white dark:bg-gray-900 mt-2 {{ (! $order->bank_name || ! $order->bank_reference) ? 'border-2 border-red-500' : '' }}">
                        <div class="flex justify-between p-4 border-b dark:border-gray-800 {{ (! $order->bank_name || ! $order->bank_reference) ? 'bg-red-50 dark:bg-red-900/20' : '' }}">
                            <p class="text-base font-semibold text-gray-800 dark:text-white flex items-center gap-2">
                                @lang('Información de Transferencia')

                                @if (! $order->bank_name || ! $order->bank_reference)
                                    <span class="badge badge-md badge-danger flex items-center gap-1 animate-pulse">
                        <span class="icon-notification text-lg"></span>
                        @lang('PAGO POR VERIFICAR (DATOS VACÍOS)')
                    </span>
                                @endif
                            </p>
                        </div>
                        ...
                        @if ($order->bank_name || $order->bank_reference)
                            <div class="grid grid-cols-3 gap-4 p-4">
                                <div class="grid place-content-start gap-1">
                                    <p class="text-gray-600 dark:text-gray-300">@lang('Banco')</p>
                                    <p class="text-base font-semibold text-gray-800 dark:text-white">
                                        {{ $order->bank_name ?: 'N/A' }}
                                    </p>
                                </div>
                                ...
                                <div class="grid place-content-start gap-1">
                                    <p class="text-gray-600 dark:text-gray-300">@lang('Referencia')</p>
                                    <p class="text-base font-semibold text-gray-800 dark:text-white">
                                        {{ $order->bank_reference ?: 'N/A' }}
                                    </p>
                                </div>

                                <div class="grid place-content-start gap-1">
                                    <p class="text-gray-600 dark:text-gray-300">@lang('Monto Pagado')</p>
                                    <p class="text-base font-semibold text-gray-800 dark:text-white">
                                        {{ core()->formatBasePrice($order->bank_amount) ?: 'N/A' }}
                                    </p>
                                </div>
                            </div>
                        @else
                            <div class="p-4 flex items-center gap-3 text-red-600 dark:text-red-400">
                                <span class="icon-information text-2xl"></span>
                                <p class="font-medium">
                                    @lang('El cliente seleccionó transferencia pero no registró los datos. Es necesario contactarlo para confirmar el pago.')
                                </p>
                            </div>
                        @endif
                    </div>
